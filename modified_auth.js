// Modified auth.js file with API URL configuration
...
async function verifyPhoneNumber() {
  try {
    ...
    // Use the API URL from config
    const response = await fetch(`${config.apiUrl}/api/phone-verification/verify`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
...
async function verifyEmail() {
  try {
    ...
    // Use the API URL from config
    const response = await fetch(`${config.apiUrl}/api/email-verification/verify-email`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-auth-token': localStorage.getItem('token')

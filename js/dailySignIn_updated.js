...
function handleDailySignIn(event) {
    ...
    const ubtReward = (Math.random() * (1.5 - 0.5) + 0.5).toFixed(2);
    ...
    showNotification(`Daily sign-in successful! You've earned ${ubtReward} UBT. You've signed in for ${consecutiveDays} consecutive days!`, 'success');
    
    // Send sign-in data to server (mock implementation)
    sendSignInDataToServer(userEmail, ubtReward, consecutiveDays);
}
 ...
 */
function sendSignInDataToServer(userEmail, ubtReward, consecutiveDays) {
    console.log(`User ${userEmail} signed in and earned ${ubtReward} UBT. Consecutive days: ${consecutiveDays}`);
